/**
 * jQuery ctypefmt plugin v0.1
 * https://github.com/toohamster/jquery-ctypefmt
 *
...
(function($) {
	...
	function cTypeDetect(str) {
		var rs = cIsJSON(str);
		if (!rs['errno']) return 'json';
		rs = cIsXML(str);
		if (!rs['errno']) return 'xml';
		rs = cIsHTML(str);
		if (!rs['errno']) return 'html';
		return 'text';
	}
	...
	XMLFmt.prototype.execute = function(text) {
	...
	var fmtXML = function(xml) {
		return (new XMLFmt()).execute(xml);
	}
	...
	function style_html(html_source, indent_size, indent_character, max_char) {
		var multi_parser = new HtmlParser();
	...
	var fmtHTML = function(str)
	{
		str =str.replace(/^\s+/, '');
        if (str && str.charAt(0) === '<')
        {
            return style_html(str, 1, '  ', 80);
        }
        else
        {
            return js_beautify(str, 1, '  ');
        }
	}

	$.cTypeDetect = function(str) {
		return cTypeDetect(str);
	}

	$.cTypeParse = function(str, ctype) {
		ctype = ctype || cTypeDetect(str);
		ctype = ctype.toLowerCase();
		if ('json' == ctype) return $.parseJSON(str);
		if ('xml' == ctype) return $.parseXML(str);
		if ('html' == ctype) return $.parseHTML(str);
		return str;
	}

	$.cTypeFmt = function(str, ctype) {
		ctype = ctype || cTypeDetect(str);
		ctype = ctype.toLowerCase();
		console.log(ctype);
		if ('json' == ctype) return fmtJSON(str);
		if ('xml' == ctype) return fmtXML(str);
		if ('html' == ctype) return fmtHTML(str);
		return str;
	}
